...
export function llmRoutes(db: Db) {
  ...
  router.get("/llms/agent-configuration.txt", async (req, res) => {
    ...
    const lines = [
      ...
      "",
      "Team structure implementation workflow:",
      "- Use POST /api/companies/:companyId/agent-hires as the canonical way for agents to create team members.",
      "- Do not use POST /api/companies/:companyId/agents from an agent runtime; that route is board-only.",
      "- Use PATCH /api/agents/:id to adjust reporting lines (`reportsTo`), role, title, capabilities, or adapter/runtime config.",
      "- Use PATCH /api/agents/:id/permissions to adjust team structure permissions when your actor is authorized.",
      "- Use POST /api/agents/:id/pause, /resume, /terminate, and DELETE /api/agents/:id for lifecycle structure changes when your actor is authorized.",
      ...
      "- If company settings require board approval for hires, agent-hires creates the pending approval flow instead of bypassing it.",
      "",
    ];
